...
log(){
progName="makeMultiScaleLists.sh"
...
if [ $# -lt 2 ]
then
  echo "Usage: ./${progName} step radius"
  exit 0
fi
...
radius=$(${BIN_DIR}/calc.pl $step/2 + 0.5)
...
java -jar ${BIN_DIR}/Gal2fk5.jar $gridFile
...
./coords2multiScaleLists.sh $gridFile_fk5 $dist $outputDir > $logFilename
